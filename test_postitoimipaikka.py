...
def test_helsingin_postinumero():
    tulos = postitoimipaikka.seek_district("00100")

    assert tulos == "HELSINKI"


def test_korvatunturin_postinumero():
    tulos = postitoimipaikka.seek_district('99999')
...
def test_postinumerot_omalla_datalla(mocker):
    ...
    mocker.patch('http_pyynto.fetch_postaldata', return_value=oma_data)
    tulos = postitoimipaikka.seek_district("90210")
